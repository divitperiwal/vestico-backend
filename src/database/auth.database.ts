...
export const saveSession = async (sessionId: string, userId: string, expiresAt: Date) => {
  const [session] = await db.insert(sessions).values({ sessionId, userId, expiresAt }).returning({
...
//User authentiaction relataed database operations
...
export const registerUserData = async (email: string, passwordHash: string, name: string) => {
  const [user] = await db
    .insert(users)
